...
**Declared and undeclared.** A binary named `ff-<name>` on PATH stays what git made it: fufu captures, hands down `FF_REPO`, `FF_CONTRACT` and `FF_SESSION`, and execs. That is the whole of an undeclared extension, and deliberately the whole of it — fufu found the thing by filename and has read nothing else about it, so there is nothing it can honestly tell an agent about the verb. Declaring is the missing half. A person runs `ff extension add <name>`; fufu asks the binary for its manifest over a `--ff-manifest` handshake, checks the contract that manifest claims against its own, and records it under the user's config directory. From then on fufu will *describe* the extension: `ff help <name>` and `ff explain <name>/<id>` delegate to the binary, its briefing line rides fufu's, its skills install beside fufu's, the neutral agent event fans out to it, the tools it produces are served beside fufu's own, and a server of its own registers beside fufu's.

What the line divides is what fufu vouches for, and nothing else. Declaring buys no capability and no environment — an undeclared extension runs from a shell exactly as it ran before, on the same three variables — and every one of those channels is fufu telling an agent that a verb exists and answers in fufu's shapes. A filename on PATH is not evidence of that, which is why it takes a person's verb and a manifest fufu has read. The registry sits under the user's config rather than in the repository for the reason a checkout does not get to install a hook: declaring is a decision about this machine, and it is not one an agent makes for itself, so `ff extension` is the shell's and no tool's.

**The envelope's key stays `ff`.** A served extension answers in the machine surface entire — one envelope on one line, `cmd` spelled `<name> <verb>`, error ids under `<name>/`, the same five exit codes — and the envelope leads with `ff` rather than with the extension's own name: `{"ff":1,"cmd":"tower brief","data":…}`. The key is the version of the *envelope*, not a signature of whoever printed it, and the printer is already named twice inside, by `cmd` and by every id it can raise. A key per extension would put two unrelated numbers in one field position — the shape of the envelope, and the extension's own versioning, which is a manifest field — and would require a reader to know an extension's name before it could tell a well-formed envelope from a stray line of JSON, which is the first question the envelope exists to answer. The cost lands where it should: tower stamps `{"tower":1,…}` today and the relay looks for `ff`, so tower's replies reach an agent as text with no structured content at all. Conforming is the extension's change to make once, against every reader of every extension learning a new key.
