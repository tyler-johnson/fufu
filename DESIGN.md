...
Neither verb guesses. The commit is named or it is `HEAD`. Attributing each hunk to the commit it belongs to — `hg absorb`'s trick, and the reason the verb carries that name — is a later integration rather than the default behavior, because a verb that guesses wrong at a distance is worse than one that asks. The long reach
