...
Neither verb guesses. The commit is named or it is `HEAD`. Attributing each hunk to the commit it belongs to — `hg absorb`'s trick, and the reason the verb carries that name — is a later integration rather than the default behavior, because a verb that guesses wrong at a distance is worse than one that asks. Lifting a commit's every change leaves the commit, empty: dropping it would be the same guess the verb had just refused to make, and a commit that says something while changing nothing is already something fufu writes. The long reach
