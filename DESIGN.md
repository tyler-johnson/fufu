...
Every working-copy state is continuously snapshotted into refs outside the visible
...
**Branches stack, and a stack is a parent link.** A branch records the *branch* it sits on, not merely the commit it forked from — a commit cannot follow its parent as the parent moves, and following is the whole point. When a parent moves its children go stale, and the same free probe that answers "would rebasing onto main conflict?" answers it one level down: `ff branch` and `ff status` say *parent moved, replays clean* or *parent moved, conflicts in two files*, in the verdicts they already spend. Knowing changes nothing on its own. `ff sync` is what applies it, to every local branch in the repository, and `ff restack` takes the name of one branch and carries the ones stacked on it. What makes either safe is that a replay is performed rather than presumed: a branch moves only when its commits replayed clean onto the base that now stands beneath it, and one that conflicts holds where it is. None of it crosses the push boundary: a cascade moves local refs and says when the commits it replaced are still on the remote, and nothing leaves the machine until `ff publish` is typed on that branch, under its lease.

The cascade is sequential, and saying so is part of the feature: moving a branch makes *its* children stale against a base that did not exist a moment earlier, so each child is replayed onto the tip its parent now has, parent before child, through the whole tree, and every verdict comes from performing that replay rather than from predicting it against the tree as it stood. A whole-tree answer is what the run produces, not what it promises in advance: a branch whose replay conflicts holds where it is, the branches above it stay put because their base did not move, and the run goes on to the next tree. All of it rides the operation of the verb that moved the first branch, so one `ff undo` takes the rewrite and its cascade back together. The exit answers the verb's own question: `ff restack` and `ff sync` ask whether the stack is lined up with what it sits on, so a hold anywhere in the run exits 3, while `ff absorb`, `ff lift`, `ff describe`, and `ff done` ask whether their rewrite landed, and it did, so they exit 0 and a hold above is disclosed the way every held rewrite is. `ff restack` is the primitive under all of it — replay these commits onto that base, in memory, hold on conflict — and the rest are aims for it: `ff sync` runs it against both things every branch answers to, with the network in front and one fetch for the run, the remote axis over every branch and then the base axis parent before child, `ff restack --onto <branch>` records a new parent before replaying, and `ff done` is restack pointed at an edit session's parent, a session being a branch temporarily inserted beneath one. A branch answers to two: the **base** beneath it, and the **remote** copy of itself. Reconciling with either is a replay that can conflict, which is why one verb covers both. Sending is not reconciling, so it is not that verb. Everything `ff sync` does is recorded and reachable from `ff undo`; a push is the one act that leaves the machine, and no operation log reaches across a wire — so `ff publish` is a separate verb, typed on purpose, and there is no knob that makes it happen by itself. Not reaching across a wire is a limit on undo's arm, not on the log's memory: a push is recorded like anything else, and the way back from one is the same verb pointed the other way — undo the commit, publish again, and the lease rolls the shared copy back to where the branch now stands. The lease it carries is the tracking ref as you last saw it, which is also why publish never fetches: refreshing that value first would ask git to guard you against a change you accepted without reading. The exit guards are properties of publishing rather than of a mode, so a held rewrite is refused.

The shared copy of a branch is the one base that can hold old versions of the branch's own work, which is what makes the remote half of sync two questions rather than one. **Have you changed this branch since you last saw its shared copy? If not, follow the shared copy wherever it went. If you have, is what it holds beyond you new work, or old versions of yours? New work replays under your commits; old versions of yours are left for publish to replace.** Git cannot ask the second question: a branch you restacked and a branch a collaborator pushed to are the same shape, and the correct answers are opposite. fufu can, from two records it already keeps. The operation log names every commit it rewrote or dropped as empty, so a shared copy holding only those is holding your past. Publish records where it left the shared copy, so a tracking tip equal to that, with nothing arriving, is a tip this repository sent, and what it holds beyond the branch is work you stepped back from rather than work arriving; that memory is a ref rather than a row on the log, because `ff undo` is a pointer move and would rewind the log straight past the one fact undo cannot reverse. Anything neither record recognizes is new work however it arrived, and replays, which never loses work. The first question is answered by reading the tracking ref before the fetch and after: a branch standing exactly where the shared copy stood is unchanged, and follows, force-push included. The lease publish carries is built from the same reading, the tracking ref as last seen, spelled once and used twice. `--no-fetch` needs no rule of its own; it reaches the same questions.

A restack replays as deep as the branch goes, and no deeper than the branch's own commits: the range stops where the branch forked from the base's history, read from the base's reflog the way `git rebase --fork-point` reads it, so a base rewritten beneath a branch does not hand its old commits back as the branch's and conflict on them. That bound applies to the base a branch follows and to its own shared copy; `--onto` aimed elsewhere keeps the plain merge base, since a transplant carries everything above the common ancestor. The futures probe caps itself because it runs at every prompt; a verb somebody typed pays the real cost instead, a cap there refusing a long branch to save an expense nobody was being charged. The working copy enters the account only when the branch it stands on is one the rewrite carries — restacking a branch you are not on is refs and objects, and moves no file at all — and when it is carried, the open change replays as the last step onto the new tip, so a restack that would conflict in uncommitted work refuses before anything moves: the same answer at the same moment, one step further out than the commits. `--onto`'s parent write rides the operation record beside the ref moves, because undoing the replay while leaving the re-aim standing would point a branch at a base it no longer sits on, which is worse than either half alone.

**Every worktree gets its own log.** A linked worktree is another checkout of the same repository: it shares one object store and one ref namespace, and until it also shared one operation chain, one undo pointer and one lock. That is correct for one tree and corrupting for two — a read in a bay appended to the shared chain just by looking, and an undo in the main worktree moved its HEAD to the bay's branch and overwrote its working copy with the bay's, which is a state git itself refuses to create. So the chain is keyed by worktree, at `refs/fufu/wt/<id>/ops`, where `<id>` is the gitdir basename git already files it under and keeps stable across `git worktree move` and `git worktree repair`. A path would survive neither.
...
drops the detachment. The short reach is at a distance, and it is one move with two spellings: content leaves a contiguous run of commits on the branch's line, `--from <revset>`, and lands in one commit, `--into <rev>` — the open change being a commit like any other, so either end may be it. `ff absorb` is that move from the open change into the commit under it; `ff lift` is that move from the commit under the open change into the open change; and each verb's word is its defaults and nothing more. The shapes are a ladder: a target below the run takes the sources folded in and each source is replayed without what moved, so one that empties is dropped and named — git's `rebase -i` squash, as `ff absorb --from HEAD~2..`; a target above the run, or inside it, takes the moved paths from a fold of whatever stood above it while the run loses them; a target of the open change leaves the worktree where it is, so the tip tree loses exactly what the open change gains, and no file moves at all. Everything between and above replays in memory and the refs move once; you never leave your tip. A descendant that depends on a moved change cannot replay without it, and holds like any other rewrite.

Neither spelling guesses. The ends are named or they are the verb's defaults, and a bare absorb whose default target would be trunk's commit refuses rather than rewrite trunk. Attributing each hunk to the commit it belongs to — `hg absorb`'s trick, and the reason the verb carries that name — is a later integration rather than the default behavior, because a verb that guesses wrong at a distance is worse than one that asks. Dropping a commit the move emptied is not the same kind of guess: what to attribute a hunk to is a question about intent, and whether a commit changes anything is a fact about its tree. The long reach
...
standing region in the working copy in one editing session — ordinary conflict
...
be pinned, because every input is a ref or the working copy; the pending rewrite
...
| `ff absorb [--from <revset>] [--into <rev>] [-m <msg>] [<paths>]` | move content out of a run of commits into one commit, descendants restacked in memory: bare, the open change into the commit under it; `--from HEAD~2..` folds a run into the commit beneath it, and the emptied sources are dropped and named; `-m` rewords the target as it lands | `commit --fixup` + `rebase -i --autosquash`; `rebase -i` with `squash` and `fixup` |
| `ff lift [--from <revset>] [--into <rev>] [-m <msg>] [<paths>]` | the same move with the other defaults: bare, the commit under the open change into the open change; `--into <rev>` lands it in a closed commit instead. Only ownership moves; no file does | `reset --soft`, then nothing |
...
| `ff restack [<branch>] [--onto <base>]` | the primitive, offline: replay a branch's commits onto the branch it sits on — yours unless you name another; `--onto` records a new parent first, which is how a branch is re-aimed. The branches stacked on it follow, parent before child, in the same operation; one that is another worktree's HEAD is skipped and named, with everything above it | `rebase --onto` arithmetic, and `--update-refs` run once per branch in the stack |
| `ff fold [<target>] [--stay]` | land the branch you stand on into another — trunk when none is named — and take the branch away: its commits replay onto the target's tip, the target fast-forwards to the result, the branch is deleted, and you stand on the target with the open change still open. Nothing is merged; the branches stacked above follow and record the target as their base. `--stay` keeps the branch sitting on the result and, when another worktree holds the target, advances it there, one operation on two chains | `rebase`, `switch`, `merge --ff-only`, `branch -d`, and the stash dance around the four |
| `ff sync [--no-fetch]` | line every local branch up with both things it answers to — reconcile with the remote, restack onto the base if it moved, cascade into the branches above: land if clean, hold if not, one operation for the run. Nothing leaves the machine; it names what is left to publish | `fetch` + `rebase`, in the right order, and prayer |
...
A handful of git's and jj's words are answered rather than parsed: `checkout`, `stash`, `pull`, `push`, `blame`, `tag`, and jj's `abandon` and `split`. Typing one is a question — how do I do the git thing here, or the jj thing? — and "unrecognized subcommand" answers a different one, so each raises a coded refusal naming the verbs that replaced it, with what was typed folded into the exits the way `ff branch <name>` folds it. This is the retired `-m` and `ff log --ops` again: a word fufu chose not to have is worth more than a word it never heard of. `blame` and `tag` earn the entry on the half git does not answer: `blame` reads history, and the work fufu is holding is the part that is not history yet; a tag is git's to make, but `refs/tags/` rides every operation's ref table, so putting back one that was deleted is `ff undo`. jj's two name acts fufu spreads over verbs it already has: what `abandon` drops is a different thing at each stage — the open change is `ff restore --all`, a session or a held rewrite is `ff done --abandon`, a closed commit is `ff lift --from` — and `split` is closing in slices, `ff commit <paths>` for the open change and `ff lift --from <rev> <paths>` for one that has closed. They are refusals and nothing else, so they capture nothing — a snapshot taken for a command that does not exist would be a row on the log for something that never ran. `co` is not among them: it is a `.gitconfig` alias rather than a word git spells, and a second spelling for one refusal is surface with no reader.
...
Run inside a repository that already exists, `ff init` means *turn fufu on here*: the same work, reported honestly as already done when it was. That is the adopt path for a repository git made, and it is why the verb is not `ff init` in the git sense at all — a `--bare` is refused, naming `ff git init --bare`, because a bare repository has no working copy and therefore nothing for a floor to hold.
...
Staged and tracked are git states fufu has no concept of. The working copy *is*
...
Hooks are the oldest mechanism and the widest, and they are two verbs rather than one because they are two contracts. `ff hook` and `ff unhook` are what a person types: flat, permanent slugs — `claude`, `codex`, `qwen`, `opencode`, `copilot`, `cursor`, `bash`, `zsh`, `fish`, `powershell` — where an unknown name is a real error and a failure is loud. A source once written is answered forever: `gemini` went as a slug when its adapter did, and `ff trigger gemini` keeps capturing from the settings file an earlier fufu wrote, through a retired protocol that has no slug, no detection, and no install. `ff trigger <source>` is what a client calls, and its contract *is* the extension point: always exit 0, never veto, fail silently, `FF_DEBUG=1` to see why, and an unrecognized source name exits 0 in silence rather than erroring. That last clause is what makes a fufu trigger safe to wire into a tool fufu has never heard of, so it is published rather than reimplemented per vendor.
...
Delivery is the same directory story the plugin already tells. Claude Code's skill rides inside the plugin fufu owns outright, and so does Codex's — its plugin under `~/.agents/plugins/fufu/` is discovered through a personal marketplace file fufu merges one entry into and otherwise leaves as found. Copilot CLI takes an Agent Plugins 1.0 plugin under a `copilot/` namespace of the same root, its marketplace shared with tower by merging rather than overwriting, and a registration in its settings; its payload names no event, so each hook entry sets one in the environment. OpenCode takes one plugin module fufu writes whole beside a skill directory of its own, and the module's system-prompt transform makes the briefing standing there — on every model call, surviving compaction — where every other client hears it once per boundary. Cursor takes a user-local plugin its CLI discovers on session start, with the skill inside. Qwen Code reads no skills directory, so it gets the briefing alone and are not told to read something that is not there — which is why the question is asked of the adapter at print time rather than assumed from the install. The skill is also printable: `ff hook --skill` writes the same bytes an install would to stdout, which is what makes the manual reachable on a client that reads no skills directory and on one fufu has never heard of — the half of the published extension point that was missing, since `ff trigger <unknown>` already invites a third party to wire capture. The block at the end of `ff --help` is what routes an agent to it, and carries the same guard the briefing does: a test parses the command it names.

A hook never vetoes on its own judgment; the wish to stop an agent from touching a branch is policy, and policy lives in config — so the one veto there is, `fufu.gitPolicy strict` for raw git, is config saying so, and it travels as JSON the client is free to ignore rather than as an exit code. `ff trigger` still always exits 0.
...
12. **Opinionated workflow, neutral repository.** fufu takes positions — replay
    over merge for your own base, and a merge only for a tree you don't own;
    malleable unpublished history; routine leased force-pushes. The
...
The machine surface is not a phase but a constraint on every one: one model with two renderers is a Phase 0 substrate decision, error ids and exit codes arrive with the verbs that raise them, sessions ride the capture floor in Phase 1, `ff watch` follows the operation log in Phase 2, and the generated surfaces — completions — land with adoption in Phase 5.
...
- **Agent adoption** — the machine surface makes fufu usable by an agent; what
  ...
  rewriting a command line. The other candidate, a typed-tool door, shipped
  as `ff mcp` in v0.12 and was removed in v0.15: every call was an ordinary
  `ff <verb> --json` child, so the shell already reached every verb with the
  same contract, and the second door bought nothing the first lacked.
