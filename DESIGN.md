...
| `ff collide [<a> <b>]` | the sideways axis: would these two branches hit each other if both landed? A read, replayed in memory against the merge base, so the answer costs nothing and changes nothing. Bare it pairs every local branch with every other; named, it asks about one pair | attempting the merge to find out, then undoing it |
