...
//! ```
//!
//! Slot 1 is reserved for the chain and is never a pin. The journal used it
//! as a pin on its first entry, which is why `git log --first-parent
//! refs/fufu/journal` ran off the root into the user's own history and kept
//! going. Base sits at a fixed slot 2 so a capture's parents are `[prev,
//! base]` — byte-identical in shape to a snapshot commit today, which is
//! what lets the existing decode rule and the existing differential
//! assertion carry over unchanged.
//!
...
pub use append::{CaptureOutcome, capture};
...
///
/// [`BRANCH_PREFIX`] is deliberately the namespace the capture chain uses
/// today, because after the cutover it means the same thing — the newest
/// thing fufu wrote on this branch. Until the cutover the two writers must
/// not run in one repository: a pointer holding a snapshot commit fails
/// [`is_op_commit`], and the append refuses rather than writing an op whose
