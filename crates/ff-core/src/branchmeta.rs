...
//! parent branch a `ff start` forked from, the session the branch is
//! sitting in while it is unfinished, the rewrite that conflicted and
//! is waiting on the branch, and the resolution session open over it.
//! Plain files under
...
pub struct BranchMeta {
    ...
    /// Set iff this branch is an unfinished session: one `ff edit` opened,
    /// or the one `ff resolve` opened, whose `onto` carries the `resolving`
    /// record naming this branch.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub session: Option<Session>,
    /// Set iff a rewrite on this branch conflicted and is waiting.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub held: Option<crate::held::Held>,
    /// Set iff a resolution of this branch's hold is open, on the session
    /// branch the record names.
    #[serde(default, skip_serializing_if = "Option::is_none")]
