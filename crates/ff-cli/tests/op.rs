...
/// Like `ff` but with the tool-call marker set, which is what every child of
/// `ff mcp` and anything it spawns carries.
fn ff_as_tool(fx: &Fixture, args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_ff"))
        .current_dir(fx.path())
        .args(args)
        .env("GIT_CONFIG_GLOBAL", null_device())
...
/// Every operation says how its invocation arrived: `shell` from the shell,
/// `tool` under the relay's marker, and `route(tool)` picks the latter out.
#[test]
fn every_operation_carries_its_route() {
    let fx = with_ops();
    let out = ff(&fx, &["op", "show", "@", "--json"]);
    assert!(out.status.success(), "{}", stderr(&out));
    assert_eq!(json(&out)["data"]["route"], "shell");
    let shell_id = json(&out)["data"]["id"].as_str().unwrap().to_string();

    fx.write("b.txt", "through the tool\n");
    let out = ff_as_tool(&fx, &["commit", "-m", "tool close"]);
    assert!(out.status.success(), "{}", stderr(&out));
    let out = ff(&fx, &["op", "show", "@", "--json"]);
    assert_eq!(json(&out)["data"]["route"], "tool", "{}", stdout(&out));
    let tool_id = json(&out)["data"]["id"].as_str().unwrap().to_string();

    // The human render names it too.
    let out = ff(&fx, &["op", "show", "@"]);
    assert!(
        stdout(&out).contains("route     tool"),
        "the route is a line of its own: {}",
        stdout(&out)
    );

    // The filter is exact, and the shell rows are not in it. Reading the log
    // is a shell invocation that captures first, so the newest row is that
    // capture; the tool's own capture and its close are the two tool rows.
    let out = ff(&fx, &["op", "log", "route(tool)", "--json"]);
    assert!(out.status.success(), "{}", stderr(&out));
    let rows = json(&out)["data"]["ops"].as_array().unwrap().clone();
    assert_eq!(rows.len(), 2, "the tool's capture and its close: {rows:?}");
    assert_eq!(rows[0]["id"], tool_id);
    assert!(rows.iter().all(|row| row["route"] == "tool"), "{rows:?}");
    assert!(rows.iter().all(|row| row["id"] != shell_id), "{rows:?}");
}
