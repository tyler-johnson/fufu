...
/// *single* object: the briefing and a `fufu.gitPolicy` correction can
...
///
/// The hook has one config-driven refusal, raw git under gitPolicy: policy
/// from config, travelling as JSON the client may ignore, and one reply
/// carries at most one reason. `ff trigger` still always exits 0.
///
...
/// `C:\Users\...\ff.exe` collapses to `C:Users...ff.exe`. Inside double
...
mod tests {
    ...
    /// backslash, and quoting costs nothing elsewhere.
    #[test]
    fn the_exe_command_quotes_the_path() {
        ...
        assert_eq!(command, format!("\"{}\" trigger test", exe_path()));
    }
