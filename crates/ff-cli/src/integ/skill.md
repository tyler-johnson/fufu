...
- `ff pull -n` says what the run would do — what would fast-forward, replay, hold, or be skipped — and writes nothing. The fetch still runs, moving only remote-tracking refs; `--no-fetch` beside it reads what is already here.
- `ff push` sends the branch you stand on under a lease: the push goes through only if the shared copy still stands where you last saw it. `ff push <branch>...` sends those branches instead, each under its own lease; a refused lease is that branch's alone, the rest go out, and the exit is 1. There is no `--all`. It does not fetch first, on purpose. It is the one thing fufu does that no operation log can take back, which is why it is a verb you type rather than a step riding inside another.
