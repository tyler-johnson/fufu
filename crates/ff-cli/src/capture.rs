...
/// Take the pre-command capture, swallowing every failure (diagnostics only
/// under `FF_DEBUG=1`). Outside a repository or in a bare one this is a
/// silent no-op. Never touches exit codes or stdout.
///
...
fn pre(prov: &Provenance) -> ff_core::Result<Vec<String>> {
    let repo = match ff_core::discover(".") {
        ...
        Err(ff_core::Error::Discover(_)) => return Ok(Vec::new()),
