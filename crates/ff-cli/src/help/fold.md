Lands the branch you are standing on onto another and takes the branch away: its commits replay onto the target's tip, the target fast-forwards to the result, the branch is deleted the way `ff branch -d` deletes one, and this worktree moves to the target with the open change still open. Trunk is the target when you name none. One operation, so one `ff undo` takes all four moves back.
