...
The source's commits replay from their fork point with the target, followed by its open change. A conflicting primary replay refuses to land. Use `ff restack --onto <target>` to record that replay as a held rewrite, then `ff resolve` and finish it before folding again. Pre-operation capture can still occur.
