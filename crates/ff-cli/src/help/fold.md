Replay the current branch onto a target, advance the target, and delete the source branch. With no target, use trunk. This worktree switches to the target with its uncommitted work still open. One `ff undo` reverses the local operation.
...
The source's commits replay from their fork point with the target, followed by its open change. A conflicting primary replay refuses to land. Use `ff restack --onto <target>` to record that replay as a held rewrite, then `ff resolve` and finish it before folding again. Pre-operation capture can still occur. A held restack or merge on the source is dropped with the fold and named; a held absorb, lift, done, or parked-change arrival refuses the fold.
