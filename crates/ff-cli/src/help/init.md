...
A `git init` leaves a repository whose operation log begins whenever some later fufu verb happens to take a floor, and whose gc guard — the config that stops `git gc` expiring fufu's own refs — is written at that same unpredictable moment. This writes both before you have typed anything else, so `ff undo` has somewhere to land from your first command onward.
...
It does not touch your shell or your agent. Those are yours, not this repository's, and `ff hook` installs them when you want them — `ff doctor` says what is wired and what is not.
