...
Commits between and above the endpoints replay in the same operation, including branches inside that range. Surviving changes keep their change IDs while commit hashes change. A conflicting primary replay records a held rewrite without landing it and exits 3; captures and metadata may still be written. `ff resolve` opens it. A held restack already on the branch stays and follows the rewrite; a held absorb, lift, or done refuses the move.
