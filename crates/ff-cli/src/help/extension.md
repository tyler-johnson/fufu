...
What declaring buys is that fufu will describe the extension: the `ff mcp` tool serves its verbs, the card names them, `ff help <name>` and `ff explain <name>/<id>` reach the binary, its briefing line rides fufu's, its skills install beside fufu's with `ff hook`, and the agent event fans out to it.
