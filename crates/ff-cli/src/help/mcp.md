...
An extension is served when it is declared. `ff extension add <name>` records the manifest, and from then on `ff <name>` through the tool is relayed the way a verb is: the child dispatches to `ff-<name>` and the envelope it printed comes back as structured content.
...
A declared extension whose manifest says `undoable: false` is refused on that route, with `usage/mcp-extension-not-undoable`: the one tool's annotations say that nothing it relays is destructive, which is honest only of an extension whose writes `ff undo` takes back.
