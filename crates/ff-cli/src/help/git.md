Snapshots first, then runs the git command. Nothing is ever translated: the command that runs is the one you typed, or none at all. This is what the shell alias runs — `alias git='ff git'`, installed by `ff hook <shell>` — so typed git keeps working exactly as it did, with a snapshot in front of it.
