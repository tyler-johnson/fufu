Attempts a snapshot, then runs a permitted Git command verbatim. A strict-policy refusal happens before capture. If capture fails, fufu warns and Git still runs. The shell alias — `alias git='ff git'`, installed by `ff hook <shell>` — routes typed Git here while that alias is active.
