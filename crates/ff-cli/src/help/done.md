...
Landing a resolution — the `ff done` that finishes `ff resolve` — runs `pre-commit` too, the way `git rebase --continue` does. `--no-verify` skips them; `--abandon` runs none, since nothing is being committed.
