The changes view: the open change (@) sits above the commit walk (●), and each commit has a change ID — the letters column `ff evolog` uses to inspect its recorded evolution. The @ row carries the ID the change will keep when `ff commit` records it in branch history. Its SHA identifies the internal open commit object; a different message, partial commit, signing, or hook can give the recorded commit a different SHA. The SHA is blank while the tree is clean and under signing.
