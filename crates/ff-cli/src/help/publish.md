Send this branch to its remote — the one thing fufu does that no operation log can take back. `ff sync` takes in; this sends.
...
- `--to <remote>` names where to send a branch that does not answer to one yet, and records the answer, so the next sync and `ff status` need no flag. It is refused for a branch that already answers somewhere else: one branch, one shared copy. With a single remote, or one named origin, you never need it.
