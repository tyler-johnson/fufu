...
Two settings are only writable from a shell. `gitPolicy` and `toolPolicy` decide what fufu refuses an agent, so a write to either through the `ff mcp` tool — a value, or `--unset`, which lowers the tier by taking the value away — is refused with `usage/mcp-policy-write` rather than turning off the thing doing the policing.
