...
No branch update is sent to the remote. `ff push` sends, and undo cannot reach that remote update. Pull's local branch and file changes are undoable; fetched objects, tracking-ref updates, and fetched tags are separate. When a branch is ahead of its shared copy, this says so and leaves it for the outgoing half.
