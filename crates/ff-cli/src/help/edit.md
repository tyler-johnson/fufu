...
The branch you came from stays exactly where it stands, its commits waiting ahead. `ff done` amends the commit with what you changed and replays them onto it. A branch name is a switch instead — the one available reading is taken and announced. Your open change stays where you stood as its open commit and comes back when the session ends. `ff resolve` opens its session the same way, over a held rewrite's conflicts.
