...
- `fufu.toolPolicy`: what `ff trigger claude` says when an agent runs `ff` in its shell while the `ff` tool is up for it. `observe` says nothing, `coach` names the tool once per session as context, and `strict` (the default) refuses with `permissionDecision: deny` and a reason carrying the tool name and the `args` to call it with. The seven shell-only verbs pass, and so does an `ff <name>` the tool will not serve; a compound command is refused by its `ff` segment. Presence means a server this client spawned is alive: the hook is silent when no marker is held for `CLAUDE_PID`.
- `ff mcp` holds a presence marker, `<cache>/fufu/mcp/<client pid>/<name>`, under an exclusive file lock for as long as it serves, so each server a client spawned has a liveness answer of its own. The lock is the liveness signal; a marker nobody holds is swept by the first hook that reads it or the next server that starts. fufu holds exactly one marker, its own: a declared extension's server is a process the client starts and fufu never sees, so a registration on disk says installed and nothing says running.
- Declared extensions. `ff extension add <name>` runs `ff-<name> --ff-manifest`, checks the contract the manifest claims against fufu's own and the name it claims against the binary's, and records it under the user's config directory; bare `ff extension` and `ff extension list` are the list, and `ff extension remove <name>` takes a name back off. The manifest names the extension's verbs and whether each reads only, whether its writes are undoable, a briefing line, skill files, event subscriptions, and an MCP server of its own; a manifest is refused whole rather than in part, and declaring the same name again replaces the record and keeps its place in the order. Declaring buys the extension no capability and no environment: an undeclared `ff-<name>` on PATH runs from a shell exactly as it always did, captured before the exec and handed `FF_REPO`, `FF_CONTRACT`, and `FF_SESSION`, and a declared one is handed the same three. What it buys is that fufu will describe the verb to an agent. `docs/agents/machine-surface.md` documents the handshake, the manifest fields, and the contract a served extension answers in.
- The `ff` tool serves a declared extension the way it serves a verb, and the card names every declared extension as `Extensions: tower (next, file, done, …)`, built from the manifest's verb list and capped in how many extensions it names, how many verbs each, and the length of the line, so a long registry cannot push the card past the client's cut. An extension the cap left off is served exactly as one on the line. An `ff <name>` nobody declared is refused with `usage/mcp-extension-undeclared` and an exit naming `ff extension add <name>`; a declared one whose manifest says `undoable: false` is refused on that route with `usage/mcp-extension-not-undoable`, since the one tool's annotations say nothing it relays is destructive. `fufu.toolPolicy` lets both of those through in the shell, so between the tool and the shell there is always one place a verb runs.
- `tools: true` on a declared extension's manifest, and the `ff-<name> --ff-tools` handshake it promises: an array of MCP tool descriptors — `name`, `description`, `inputSchema`, `annotations` — as one envelope on one line, recognized before anything else on the command line the way `--ff-manifest` is and answering outside a repository. A promise and not a list, on `briefing`'s static-or-produced precedent, so an extension generates its tools from the definitions its own CLI is built from and there is no second spelling to keep in step. `readOnlyHint` and `destructiveHint` are both required where MCP leaves them optional, because a produced tool is offered on what it says about itself; a descriptor with an empty name, a name two descriptors share, an `inputSchema` that is not an object, or both hints true is refused, and the list is refused whole rather than in part. The ask is time-boxed at a second where the manifest handshake is not, since its caller is a server starting up with nobody there to interrupt a binary that hangs.
- `ff mcp` lists a tool per descriptor beside its own, named `<extension>__<tool>`, asked for once when the server starts and held for the life of the connection. A call routes back through the same child as `ff <name> <verb> …`, so capture, `fufu.gitPolicy`, sessions, and error ids hold as they do for the args array. The arguments object becomes the command line: a property the schema's `positional` array names is a bare word in that order, every other is a long option spelled verbatim — a true boolean as `--key`, false and null as nothing, an array by repeating the flag. A gap in the positionals ends the line rather than shifting it, and a value a command line cannot spell, an object or an array inside an array, is a protocol error before anything runs. A failed, missing, or hanging handshake is silence: fufu serves its own tool and says nothing. The card names no produced tool, since each is already in the client's list carrying a description of its own.
- `usage/mcp-extension-not-undoable` is the args array's refusal alone. An `undoable: true` extension gets its verbs relayed there and its produced tools listed beside them; a non-undoable one gets its produced tools, which carry annotations of their own, and `verbs` is what the relay reads either way. The refusal now names the `<name>__<tool>` that does serve the verb and a shell, in place of an MCP server of the extension's own. `fufu.toolPolicy` keeps letting a non-undoable `ff <name>` through the shell: the registry records that an extension promised tools and never which verb each covers, so a refusal naming a tool that may not be there would leave the verb nowhere to run.
- `ff help <name>` returns a declared extension's own help as the page, and `ff explain <name>/<id>` hands the id to `ff-<name> explain`. Both delegate only for a name the registry carries; an undeclared one gets the unrecognized-subcommand error it always got.
- `ff doctor`'s extension rows: one per declared extension, ok when the version and contract recorded at `ff extension add` still match what the binary answers, and a `WARN` on drift, on a binary gone from PATH, or on a failed handshake. A declared extension's own MCP server folds into the same row — registered, stale, hand-written, or a fixable `WARN` when a client's hook is wired and the server is not — and an `extensions` info row names every undeclared `ff-<name>` found on PATH and every registration left behind for a name nothing declares. An extension whose manifest promises tools is asked for them too, a second spawn beside the manifest handshake, and the row names what came back — `produces 2 tools: board, file` — or `WARN`s that the handshake failed, which is the only place a promise the binary never kept shows.
- A declared extension contributes one line to fufu's briefing: the text its manifest carries, or the stdout of `ff-<name> briefing` run at print time in the event's own directory when the manifest asks for that. It rides the same boundaries and the same once-per-audience marker fufu's own notice does, is capped at 240 characters and dropped past it, and an extension that is missing, broken, or slow costs the briefing nothing.
- A declared extension's skill files ship with the hook install: `skills/<name>/` beside `skills/fufu/` inside the Claude plugin and under `~/.codex/skills/`, read from wherever the manifest named them, absolute or relative to the binary's own directory. A named file that is missing, unreadable, or too large is left out rather than failing the install. `ff hook --skill <name>` prints one, rerunning the install rewrites the directory whole, and `ff unhook` removes it for every extension still declared. Cursor and Gemini read no skills directory and get the briefing line alone.
- The neutral agent event fans out to the declared extensions that subscribed to its kind, after the capture and never before it: `ff-<name> trigger` in the event's own directory with the event as one JSON object on stdin and the same three variables, and whatever `context` its reply carries merged into the one reply the client already gets — fufu's lines first, then each subscriber in the order it was declared. A subscription names one of the six event kinds, and `BeforeTool` also requires a `matcher`, the tool names it wants with `|` between them, matched whole and case-sensitively rather than as a regular expression; a matcher carrying anything else is refused at `ff extension add`. The whole fan-out is time-boxed at 500ms, split evenly over the subscribers not yet asked, so one slow extension cannot starve the ones declared after it. A subscriber exits 0 whatever happened, is silent, and cannot veto.
- The agent event's wire shape and the `trigger` reply shape, documented as machine surface: ten fields, every one always present with `null` where it does not apply, so a handler indexes rather than probes. `tool` and `path` are new, for a subscriber woken on a tool call that has to know which tool and which file.
- `ff hook claude`, `ff hook codex`, `ff hook cursor`, and `ff hook gemini` register a declared extension's own MCP server beside fufu's when its manifest names one: `mcpServers.<name>` in the JSON files and a second table inside the marked block in Codex's TOML, with the command, arguments, and environment the manifest asked for. `ff unhook` takes them back with fufu's own, and a registration under that name written by hand is reported and left alone. The `mcp` field is for what only a live process holds — resources, notifications, subscriptions, session identity, a warm cache; typed tools alone are `tools` and need no server.
- `ff config` refuses a write to `fufu.gitPolicy` or `fufu.toolPolicy` through the `ff` tool with `usage/mcp-policy-write`, naming a shell as the place to make it, so an agent cannot lower the tier it is being policed by; `--unset` counts as a write. Reading is untouched, and every other setting writes through the tool as before.
- The error ids `extension/bad-manifest`, `extension/bad-tools`, `extension/delegate-failed`, `extension/handshake-failed`, `extension/name-mismatch`, `extension/not-declared`, `extension/not-found`, `extension/registry-unreadable`, `extension/registry-unwritable`, `extension/tools-failed`, `extension/unsupported-contract`, `usage/mcp-extension-undeclared`, `usage/mcp-extension-not-undoable`, and `usage/mcp-policy-write`.
...
- Two produced tools can land on one name. `<extension>__<tool>` cannot collide across extensions on its own, but an extension name may itself carry `_`, so `a__b` producing `c` and `a` producing `b__c` meet at `a__b__c`. The first extension the registry declared keeps the name and the later tool is dropped from the list without a word; `ff doctor` names the tools each binary produced and not that one was dropped.
- A declared extension's tool list is asked for once, when `ff mcp` starts, and held for the life of the connection, the way the registry itself is read once. An extension that gains, loses, or renames a tool is not served differently until the client restarts the server.
- A subscriber's injected `context` is uncapped. fufu's own briefing line is dropped past 240 characters; a fan-out reply's `context` is taken whole, so an extension can put as much in front of the agent as it likes.
- `ff doctor` compares a declared extension's recorded version against what its binary answers, so an extension that edits a skill file without bumping its version leaves the installed copy drifting with nothing to report it. Rerunning `ff hook <slug>` rewrites the directory and refreshes it.
- A manifest that changes its `mcp.args` leaves the old registration where it is. The ownership test that lets an install rewrite an entry is exactly the one a changed argument list fails, so `ff doctor` reports the entry as stale and `ff doctor --fix` will not repair it — remove it and run `ff hook <slug>` again.
- `ff extension remove <name>` before `ff unhook <slug>` leaves that extension's residue behind, because unhook takes back only what is still declared: `~/.codex/skills/<name>/`, since `~/.codex/skills/` is not a directory fufu owns outright and it prunes nothing it cannot name, and `mcpServers.<name>` in Cursor's and Gemini's files, since a JSON key has no whole-file rewrite the way Codex's marked block does. `ff doctor` reports the orphaned registration as info; nothing reports the skill directory. Unhook first, or remove both by hand.
