...
- `--fields <list>` on every `--json` verb projects `data` to comma-separated dotted paths, mapping over arrays and keeping nesting; the envelope's `ff` and `cmd` stand. A path that matches nothing is refused with `usage/no-such-field`, checked against the payload actually produced, and `--fields` without `--json` with `usage/bad-flags`. See [JSON output and scripting](docs/agents/machine-surface.md).
...
- [`ff resolve`](docs/reference/cli/resolve.md) on a branch with no hold whose commits hold a merge of its base takes the base in by one fufu-authored merge commit, two parents with the branch's tip first, a change id, and `commit.gpgsign` honored. A conflicting auto-merge records a `merge` hold and opens the resolution session in the same operation, exit 3; `ff done` lands the merge and `ff done --abandon` closes the session and keeps the hold. A linear branch behind its base refuses and names `ff restack`; up to date says so. JSON gains `merged` on `resolve`, `held` and `merging` inside `resolve`, and `held.verb` reads `merge` in `ff status`.
- `--resolve` on [`ff restack`](docs/reference/cli/restack.md), [`ff pull`](docs/reference/cli/pull.md), and [`ff merge`](docs/reference/cli/merge.md) opens the resolution session when the replay on the branch underfoot conflicts: the hold is recorded and HEAD moves onto the session in the same run, exit 3 still, and `ff done` lands it. A hold on any other branch is named as before. `fufu.onConflict` (`hold`, the default, or `resolve`) makes it standing and `--no-resolve` holds for one run; the pair together is a usage error. Two undos take back a restack's or merge's open, the switch then the session with its hold; three take back a pull's, the switch, the session, then the pull. JSON gains `resolve` at the top level of each verb's envelope, `ff resolve`'s own report, beside `held`.
- [`ff merge <branch>`](docs/reference/cli/merge.md) takes a local or remote-tracking branch into the current one by one fufu-authored merge commit, two parents with the branch's tip first, a change id, `commit.gpgsign` honored, `-m` for the message; the open change rides it and one `ff undo` takes it back. A branch with nothing of its own fast-forwards. The base, recorded parent or trunk, is refused with `merge/base`: `ff pull` and `ff restack` replay onto it. A conflicting auto-merge records a `merge` hold, exit 3; `ff resolve` opens it and `ff done` lands the merge. JSON: `merge` carries the report with `fast_forward` and `parents`, `held` on a conflict. `merge` leaves the answered words: the coaching line for `git merge` names `ff merge`, and `ff git merge` stays the passthrough for a fast-forward.

### Changed

- [`ff done --abandon`](docs/reference/cli/done.md) on a resolution session closes the session and keeps the hold: the session branch goes, HEAD returns to the held branch with its parked change, and `ff resolve` opens a fresh session over the standing hold. It used to drop the hold too; that stays `ff resolve --abandon`'s meaning, from the session or the held branch. `held/moved` offers `ff done --abandon` as its exit. JSON gains `done.held` with `verb` and `onto` when the hold stands.
...
- A merge inside a rewritten range is carried instead of refused by every rewrite: [`ff restack`](docs/reference/cli/restack.md), [`ff fold`](docs/reference/cli/fold.md), [`ff done`](docs/reference/cli/done.md), [`ff absorb`](docs/reference/cli/absorb.md), [`ff lift`](docs/reference/cli/lift.md), the cascade, and the remote axis of [`ff pull`](docs/reference/cli/pull.md). Its parents are mapped through the rewrite, re-merged, its own change (what [`ff show`](docs/reference/cli/show.md) measures) applied on top, and a parent that ends up beneath another dropped. A merge left with one parent and no change is dropped as empty; one left with a change becomes an ordinary commit and the report says so under `flattened`. Under `--onto`, a parent below the range that the new base holds by change id maps to its rewritten self. The `rewrite/merge-in-range` refusal is gone, with its `ff explain` entry. Pull's base axis leaves a branch whose commits hold a merge of its base where it stands and names it, the branch underfoot included, with `ff restack` and `ff resolve` as the two doors; a merge of another tree is carried like any commit. `ff status` names the same two doors on its base line, and the coaching line for `git merge` names `ff merge`, `ff pull`, and `ff resolve`.

### Fixed

- [`ff done`](docs/reference/cli/done.md) on a resolution whose fix uncovered the next conflict, a step the session never showed or a replay that tangled again, refused with `held/unresolved` and left no way forward but abandoning. It now rolls the session to another round: the fixes so far are kept as the steps' resolutions, the session's tip moves to a commit carrying the next conflict's markers, the working copy shows them, and nothing lands until a round leaves no conflict, exit 3 each round. The roll is one operation and one `ff undo` steps back a round. `held/unresolved` now names markers left standing only. JSON gains `rolled` on `done`, with `fixed`, `conflicts`, `dropped`, and the session's `at`; `ff status` counts the round's files.
...
- [`ff hook opencode`](docs/reference/hooks/opencode.md): OpenCode, wired by one plugin module fufu owns whole at `<config>/opencode/plugins/fufu.js`, with the skill beside it. The plugin captures before every tool call and puts the briefing in the system prompt of every model call, so it survives compaction. A `fufu.js` fufu did not write is reported as hand-written and left alone.
- [`ff hook copilot`](docs/reference/hooks/copilot.md): GitHub Copilot CLI, wired by an Agent Plugins 1.0 plugin at `~/.agents/plugins/copilot/fufu/` with the skill inside, an entry in the marketplace file beside it, and a registration in `~/.copilot/settings.json`. Copilot loads it live; the briefing arrives as `additionalContext`.
- [`ff hook qwen`](docs/reference/hooks/qwen.md): Qwen Code, wired by five hook events merged into `~/.qwen/settings.json`. No skill.
- `hook/failed` for a client file or plugin directory that could not be written or did not read back, and `hook/malformed` for a client file that is not the JSON object its client reads. Both leave the file untouched.
- A shell verb under Codex, Qwen Code, OpenCode, Copilot CLI, or Cursor reads that client's session variable (`CODEX_SESSION_ID`, `QWEN_CODE_SESSION_ID`, `OPENCODE_SESSION_ID`, `COPILOT_AGENT_SESSION_ID`, `CURSOR_CONVERSATION_ID`) the way it reads `CLAUDE_CODE_SESSION_ID`.

### Changed

- [`ff hook codex`](docs/reference/hooks/codex.md) writes a plugin at `~/.agents/plugins/fufu/` with the skill inside, merges one entry into `~/.agents/plugins/marketplace.json`, and runs `codex plugin add fufu@<marketplace>` when `codex` is on `PATH`, printing the command otherwise. Codex gains `SessionStart`, `Stop`, and `SessionEnd`. The manifest's version carries a hash of the hooks table, so a rewritten table or a moved binary is a new version to Codex's cache. Once the plugin verifies, the install strips what an earlier fufu wrote under `~/.codex/`: the `hooks.json` entries, `skills/fufu/`, and the marked `config.toml` block.
- [`ff unhook codex`](docs/reference/cli/unhook.md) removes the plugin directory and the marketplace entry, and no longer opens `~/.codex/config.toml`.
- [`ff hook cursor`](docs/reference/hooks/cursor.md) writes a user-local plugin at `~/.cursor/plugins/local/fufu/` with the skill inside, wiring `preToolUse` on `Shell|Write|Delete`, `sessionStart`, and `sessionEnd`; Cursor CLI discovers it on a new session. An install an earlier fufu merged into `~/.cursor/hooks.json` reads as wired and stale, and `ff hook -u` migrates it. `beforeSubmitPrompt` and `stop` are not wired: Cursor CLI 2026.09.10 gates them on user settings. Cloud agents get no user-local hooks.
- `ff hook -l --json` reports an unreadable client file as `{"state":"unavailable","complaint":…}` and a hand-written file as `{"state":"hand-written","at":…}`.
- With signing enabled, the `@` row on `ff status`, `ff log`, and `ff show` shows the open commit's sha, and `@` resolves to it; the close signs and lands a different sha, as it does for `ff commit -m` or a partial close. `open.pending` in `--json` follows.

### Removed

- `ff commit`'s `re-minted:` line and the `reminted` field in its `--json`. A close that lands a commit other than the open one is the ordinary case and says nothing about it.
- `ff hook gemini` and `ff unhook gemini`: Gemini CLI's hook names diverged from the family, and `~/.gemini/settings.json` is never touched. `ff trigger gemini` still captures and briefs for entries an earlier fufu wrote there.

### Fixed

- [`ff hook claude`](docs/reference/hooks/claude.md) on Windows wrote the binary's path unquoted, and Git Bash collapsed the backslashes, so every wired event exited 127 while `ff hook -l` still said wired. The path is quoted now; `ff hook claude` or `ff hook -u` rewrites an existing install.
