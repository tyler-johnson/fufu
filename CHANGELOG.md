...
- A merge inside a rewritten range is carried instead of refused by every rewrite: [`ff restack`](docs/reference/cli/restack.md), [`ff pull`](docs/reference/cli/pull.md), [`ff fold`](docs/reference/cli/fold.md), [`ff done`](docs/reference/cli/done.md), [`ff absorb`](docs/reference/cli/absorb.md), [`ff lift`](docs/reference/cli/lift.md), and the cascade. Its parents are mapped through the rewrite, re-merged, its own change (what [`ff show`](docs/reference/cli/show.md) measures) applied on top, and a parent that ends up beneath another dropped. A merge left with one parent and no change is dropped as empty; one left with a change becomes an ordinary commit and the report says so under `flattened`. Under `--onto`, a parent below the range that the new base holds by change id maps to its rewritten self. The `rewrite/merge-in-range` refusal is gone, with its `ff explain` entry.
