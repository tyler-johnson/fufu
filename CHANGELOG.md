...
- Every rewrite (restack, fold, done, absorb, lift, the cascade, and pull's remote axis) carries a merge in its range instead of refusing with `rewrite/merge-in-range`: its parents are re-merged and its own change applied on top. A merge left with one parent becomes an ordinary commit, reported under `flattened`. A merge that resolved a conflict keeps the resolution, and a conflict beneath it on either parent passes through to the session.
