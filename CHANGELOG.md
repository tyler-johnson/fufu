...
- [`ff merge <branch>`](docs/reference/cli/merge.md) takes a local or remote-tracking branch into the current one by a merge commit, or fast-forwards when the branch has nothing of its own. The base is refused with `merge/base`: `ff pull` and `ff restack` replay onto it. A conflicting auto-merge records a `merge` hold, exit 3, that `ff resolve` opens and `ff done` lands.
- [`ff resolve`](docs/reference/cli/resolve.md) on a branch whose commits already hold a merge of its base takes the base in by another merge commit rather than refusing. A linear branch behind its base still names `ff restack`.
- `--resolve` on [`ff restack`](docs/reference/cli/restack.md), [`ff pull`](docs/reference/cli/pull.md), and `ff merge` opens the resolution session in the same run when the branch underfoot conflicts, exit 3 still. `fufu.onConflict` (`hold`, the default, or `resolve`) makes it standing and `--no-resolve` holds for one run.
- [`ff diff`](docs/reference/cli/diff.md) takes revisions: `-r <revset>` shows a connected set's whole patch, and `--from <rev>`/`--to <rev>` two points. A gapped, forked, or two-headed set is refused with `usage/revset-not-a-range`.
- One view set on `ff diff`, [`ff show`](docs/reference/cli/show.md), and [`ff log`](docs/reference/cli/log.md): `--stat`, `--name-only`, and `-U <n>`, with `ff log -p` hanging each row's patch under it and `ff show --no-patch` stopping at the message. `ff log --body` prints each row's message body.
- `--fields <list>` on every `--json` verb projects `data` to comma-separated dotted paths. A path that matches nothing is refused with `usage/no-such-field`. See [JSON output and scripting](docs/agents/machine-surface.md).

### Changed

- A merge inside a rewritten range is carried instead of refused by every rewrite: restack, fold, done, absorb, lift, the cascade, and the remote axis of pull. Its parents are mapped through the rewrite and re-merged, with its own change applied on top; a merge left with one parent becomes an ordinary commit and the report says so under `flattened`. The `rewrite/merge-in-range` refusal is gone. Pull's base axis leaves a branch that holds a merge of its base where it stands and names `ff restack` and `ff resolve` as the two doors.
...
- `ff done` on a resolution whose fix uncovered the next conflict refused with `held/unresolved` and left no way forward but abandoning. It now rolls the session to another round with the next conflict's markers in the working copy, exit 3 each round, and lands once a round leaves no conflict. One `ff undo` steps back a round.
...
- A merge whose auto-merge conflicts is carried against its first parent, so a re-merge that conflicts in the same region refuses at the merge with `held/rewrite-conflict` rather than holding.
