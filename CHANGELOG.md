...
- [`ff pull`](docs/reference/cli/pull.md) on a branch whose commits hold a merge refused the run with `rewrite/merge-in-range` and discarded it, local trunk included. The branch is now left alone and named, and the rest of the run goes on: trunk follows its remote and the other branches move.
