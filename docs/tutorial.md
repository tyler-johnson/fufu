...
<div class="demo cast" data-cast="../assets/tutorial/line-up-then-send.cast">
  <noscript><img src="../assets/tutorial/line-up-then-send.gif" alt="ff sync taking in a teammate's commit, then ff publish pushing the branch"></noscript>
</div>
...
Every publish carries a lease: it goes through only if the shared copy still stands where you last saw it. In git's terms this is a force-push under a lease, `--force-with-lease`.
...
If somebody pushed to your branch since, nothing is sent and nothing is lost — `ff sync` takes their work in, and you publish after. [The push boundary](concepts/push-boundary.md) covers leases, rollback, and `--dry-run`.
