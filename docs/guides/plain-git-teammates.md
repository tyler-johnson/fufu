...
`ff hook bash` (or `zsh`, `fish`) installs `alias git='ff git'` in your shell, so typed git lands on the fufu surface by spelling you already have as muscle memory. The boundary is execution path: aliased git is captured first and absorbed as fufu's own, while anything that resolves git on PATH — a GUI, a script, a teammate — stays outside and is absorbed lazily as above.
