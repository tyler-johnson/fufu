...
The commit and everything above it now have new commit hashes. Their trees are unchanged and their change IDs are preserved. Any branch sitting inside the restacked range comes along with it.
...
`HEAD~2..HEAD` selects the last two commits on this branch, so they fold into the commit below them. Keep the upper bound: an unbounded `x..` ranges over visible history, not just this branch. A source the move empties is dropped, and the report names it; `-m` gives the target its new message:

```console
$ ff absorb --from HEAD~2..HEAD -m "lexer: numbers, signed and floating"
...
A run of commits comes back out the same way. `--from HEAD~2..HEAD` names the last two commits on this branch, and both are open again, dropped from the history and named:
...
[`ff collide`](../reference/cli/collide.md) asks whether two branches would conflict if both landed. Its comparison runs in memory without changing branches, the index, or worktree files. Capture, auto-fetch, and maintenance can still run around it. A second branch, `renamer`, has appeared beside `lexer`, and both touched `src/main.rs`:
...
```console
$ ff op trim -n
nothing to drop (37 operations kept)
```
...
$ ff op trim -n
would drop 37 of 40 operations
  ff/sunny-cliff: branch is gone — pointer removed
  ff/warm-quail: branch is gone — pointer removed

$ ff op trim
dropped 37 of 40 operations — previous tip saved at refs/fufu/wt/main/trash/@ops until the next trim
  ff/sunny-cliff: branch is gone — pointer removed
  ff/warm-quail: branch is gone — pointer removed
dropped data frees after gc
```

`-n` previews retention without dropping operations; a pre-command capture and update maintenance can still run. The named branches are the two editing-session branches `ff edit` minted earlier — the sessions ended and the branches are gone, so their pointers into the log go too. A real trim saves the pre-trim tip to a trash ref before removing operations, making the last trim recoverable until the next trim.
...
```console
$ ff history
@   ed8faf63b188    0s ago  now   trim: dropped 37 operation(s)
↓1  8287e7bf537d    0s ago  undo  switch from lexer to main
↓2  10cadb73d11a    0s ago  undo  switch from renamer to lexer
    (the floor)
```
...
When `commit.gpgsign` is enabled, fufu signs rewritten history commits, including replays. Without signing enabled, an old commit's signature does not carry over to the rewrite. Internal open and capture commits are unsigned; [commit signing](../reference/signing.md) explains that boundary.
...
The rewrites above changed local branch history. Sending those changes is a separate push, whose remote effects undo cannot reach. [The push boundary](../concepts/push-boundary.md) explains the lease and the team's role in deciding whether shared history may be rewritten.

The rewrite verbs accept pushed commits. Push the branch, then rewrite a commit the remote holds: fufu reports that it was published, without sending the rewrite.
...
Sending that rewrite is [`ff push`](../reference/cli/push.md)'s job. Its lease requires the remote ref to match the expected tip, and replacing commits also checks fufu's seen record. Send it when your team's policy allows the rewrite:
...
This refusal protects an unseen remote tip. It does not enforce ownership or append-only history: fufu can rewrite pushed commits and send those rewrites, including on `main`, when the lease and server-side rules allow it. Pull can follow a remote rewrite or drop a local version superseded by the same change ID; that is reconciliation, not an immutability policy.

Use the team's policy to decide whether a published rewrite should be sent, and server-side branch protection to enforce that policy. A [held rewrite](../concepts/held-rewrites.md) separately blocks pushing its branch until the hold is resolved or abandoned.
