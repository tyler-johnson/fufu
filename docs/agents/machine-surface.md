...
| `undoable` | boolean | yes | Whether every write the extension makes is captured by fufu and taken back by `ff undo` — true only when it writes through fufu's own verbs. The one tool carries a single set of annotations over everything its args array relays, and they say that nothing it relays is destructive, which is honest only of an undoable extension, so `false` is refused there with `usage/mcp-extension-not-undoable`. What it loses is that one route. The manifest still parses and the extension is still declared — it is on the card, `ff help <name>` still delegates, and the tools it produces are listed and called under annotations of their own — and the shell refusal lets `ff <name>` through, because that is the one place the verb is certainly still runnable. |
...
| `tools` | `true` | no | Whether the extension produces MCP tool descriptors, asked for with `ff-<name> --ff-tools` and typed in the next section. It is a promise and not a list: a list written here would be a second spelling of the extension's own CLI, kept in step by hand, and stale the moment the binary moved on. Absent, and `false`, mean the extension produces none. |
| `mcp` | object | no | An MCP server of the extension's own, registered as `mcpServers.<name>` beside fufu's when a client is hooked: `command`, a string, `args`, an array of strings, and an optional `env` object. This is where an extension wanting what only a live process can hold goes — resources a client attaches and re-reads, a notification when state moves, a subscription, session identity across calls, a warm cache. Typed tools alone need none of that, and are `tools` above. |
...
### The tool list, and the `--ff-tools` handshake

`ff-<name> --ff-tools` prints the MCP tools the extension produces, as an ordinary envelope on one line, exiting 0. The flag is recognized before anything else on the command line, answers outside a repository, and takes no other argument, exactly as `--ff-manifest` does; nothing is handed down but `FF_NONINTERACTIVE`, because an extension needs neither the repository nor the contract to say what tools it has. It is asked of every extension whose manifest says `tools: true`, undoable or not, because a produced tool is offered on what it says about itself; what comes back is served beside fufu's own tool, under a name fufu namespaces per extension so two extensions cannot collide by both producing a `list`.

The list is asked for rather than written down for one reason: the extension generates it from the definitions its own CLI is already built from, so there is no second spelling to keep in step and drift is impossible rather than policed. The same rule `briefing` draws with `true`.

**This handshake is time-boxed and the manifest one is not.** `ff extension add` and `ff doctor` are verbs a person typed and can interrupt. This one is asked by a server starting up with nobody in front of it, where a binary that hangs would hang the server before it ever served anything, so an extension has about a second to answer and gets no more. An extension with real work to do to produce its list should not be doing it here.

The envelope's `data` is an array of descriptors, here pretty-printed:
...
| field | type | required | meaning |
| --- | --- | --- | --- |
| `name` | string | yes | What the tool is called, bare — `board`, not `tower__board`. Namespacing is fufu's, and a name that arrived namespaced would be namespaced twice. Spelled in the characters the four clients spell a tool with: ASCII letters and digits, `-` and `_`, and nothing else. Two descriptors may not share one. |
| `description` | string | yes | What the tool does, which is the whole of what an agent reads before it calls one. Required and non-empty, where MCP leaves it out. |
| `inputSchema` | object | yes | The JSON Schema a call's arguments are shaped by. `"type": "object"`, because a call's arguments arrive as an object and a schema describing anything else describes something no call can carry. |
| `annotations` | object | yes | What the tool says about itself, in MCP's own hints: `readOnlyHint` and `destructiveHint`, both required; `idempotentHint`, `openWorldHint` and `title`, all optional. A tool claiming to be read-only and destructive at once is refused, since it has said two things. |

The field names are MCP's rather than the manifest's own snake case, because a descriptor is MCP's object: an extension that has one already copies it across. Unknown fields are tolerated and read past — dropped, where a manifest's are kept, because nothing records a descriptor and there is no round trip for one to survive. The list is refused whole rather than in part, because a tool an agent can call by a name that is sometimes there is worse than one it cannot call at all, and a list that promised tools may not come back empty.

**Both hints are required because a produced tool is offered on what it says about itself.** The one `ff` tool carries a single set of annotations over every verb it relays, and "nothing here is destructive" is honest only when `ff undo` covers all of it — which is why the args-array route serves an extension only when its manifest says `undoable: true`. A produced tool states its own two hints and is honest on its own, so it does not need that blanket promise. Both routes stand: an `undoable: true` extension gets its verbs relayed in the args array and its produced tools listed beside them, a non-undoable one gets its produced tools, and `verbs` is what the relay reads either way. Neither route is given up to gain the other, and no declared extension is left with nowhere to run: a shell runs `ff <name>` in every case, which is why the `fufu.toolPolicy` refusal lets a non-undoable one through.

**The name a client sees is `<extension>__<tool>`** — `tower__board` for tower's `board` — which is the shape MCP itself uses when a client prefixes a server's tools. Two extensions cannot collide by both producing a `list`, since the extension is in the name. An extension name may itself carry `_`, so two namespaced names can still meet; the first extension the registry declared keeps the name and the later tool is not listed. fufu's own tool keeps its bare `ff`, which the `fufu.toolPolicy` refusal spells.

**The list is asked for when the server starts and held for the life of the connection**, the way the registry is read once per process: what was advertised at handshake is what answers until the client closes, and a restart is what picks up an edited extension.

**A tool's bare name is the verb it calls, and its arguments become that verb's command line.** `tower__board` with `{"branch": "main"}` runs `ff tower board --branch main --json`. Every property is spelled as a long option, verbatim, with no case or underscore translation, because the extension generates the schema from the definitions its own flags come from: a true boolean as `--key` alone, a false one and a null as nothing at all, anything else as `--key <value>`, and an array by repeating its flag per item. A value a command line has no spelling for — an object, or an array inside an array — is refused as a protocol error before anything runs. `inputSchema` may carry one keyword of fufu's own beside JSON Schema's: `positional`, an array of property names spelled as bare words, in that array's order, before every option. A positional left out ends the line there rather than shifting the words after it onto the wrong argument.

A call that arrives through a produced tool is an ordinary invocation of the extension, so the five rules above hold for it unchanged — the envelope key, `cmd` spelled `<name> <verb>`, ids under `<name>/`, the exit codes, and `--json` last on the line.

A handshake that fails costs the agent nothing and says nothing, on `ff trigger`'s doctrine: fufu serves its own tool, the extension's verbs are relayed exactly as they were, and what is lost is the tools it promised. `ff doctor` is where that shows.
