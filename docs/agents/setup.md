...
The slugs are `claude`, `codex`, `cursor`, `gemini`, plus `bash`, `zsh`, and `fish` for the shell alias and prompt hook; bare [`ff hook`](../reference/cli/hook.md) reports what it detects and asks, and `ff hook --all` takes everything without asking. For Claude Code the installer writes a plugin directory fufu owns outright; for the other clients it merges entries into their own settings file, and it never touches a line you wrote yourself.
