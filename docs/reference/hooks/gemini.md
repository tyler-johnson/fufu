...
A fufu before v0.15 also registered the retired `ff mcp` verb as `mcpServers.fufu` in the same file. The next `ff hook gemini` removes that key, says so, and leaves every other setting as it was; a `fufu` entry that runs something else was written by hand and is left alone.
