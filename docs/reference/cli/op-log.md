...
The argument is the set language over operations: the same operators as [`ff log`](log.md), reading the other address space, and positional here for the same reason an operation id is positional in [`ff op show`](op-show.md) — the position differs only in how many members it accepts.
