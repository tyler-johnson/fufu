...
The push carries a lease: it goes through only if the shared copy still stands where you last saw it. If somebody pushed since, nothing is sent and nothing is lost — take their work in first, then send. What you last saw is fufu's own record, written by the verbs that show you the copy — `ff pull`, [`ff switch`](switch.md) onto a remote's branch, [`ff clone`](clone.md), and the push itself — so a fetch behind fufu's back (an editor's, [`ff git fetch`](git.md), `ff pull --dry-run`) moves the tracking ref and not the lease, and a push after one is refused before the wire. This verb does not fetch on its own, since a lease is only worth something as the tip you last looked at. A branch with no record yet — one from before fufu kept it, or whose upstream git set — pushes when the push only adds commits to the copy, and is asked to pull once otherwise.
