...
Every operation on the change you have open, newest first — the drill-in behind the letters column in [`ff log`](log.md). This is where a lost hour is found: each row is a whole worktree, and [`ff restore --at-op <id>`](restore.md) brings any of them back. `ff ev` is the short spelling.
