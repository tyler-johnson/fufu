...
Bring every local branch up to date with the two things it answers to: the base it sits on, and the shared copy of itself on the remote. One fetch, then each branch is replayed onto whatever moved, and the whole run is one operation — one [`ff undo`](undo.md) puts every branch and the working copy back.
