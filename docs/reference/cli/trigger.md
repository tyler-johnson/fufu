...
A subscriber inherits this page's doctrine whole: it exits 0 whatever happened, it is silent, and it cannot veto. The time box is fufu's, half a second shared across the whole fan-out. Which events an extension asks for is its manifest's business; [`ff extension`](extension.md) is the verb that declares one.
