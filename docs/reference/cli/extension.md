...
A name that was never declared is refused rather than answered as done. [`ff undo`](undo.md) does not reach any of this either: the list is per machine and lives outside every repository, so the way back is `ff extension <name>` again.

## Usage

```
Usage: ff extension [OPTIONS] [name]
