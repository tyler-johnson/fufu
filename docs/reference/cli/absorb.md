...
Commits between and above the endpoints replay in the same operation, including branches inside that range. A conflicting primary replay records a held rewrite without landing it and exits 3; captures and metadata may still be written. [`ff resolve`](resolve.md) opens the conflicts. A held restack or merge already on the branch stays and follows the rewrite; a held absorb, lift, done, or parked-change arrival refuses the move.
