...
The fetch still runs. Without it the report could not say what the shared copy holds, and a fetch writes the remote-tracking refs under `refs/remotes/<remote>/` and the objects behind them, what [`ff git fetch`](git.md) writes, and touches no local branch, no file, and no operation. `--dry-run --no-fetch` reads what you already have and writes nothing at all. The exit is 3 when a branch would hold, the same as a real run, so a script can ask before it pulls.
