...
`gitPolicy` and `toolPolicy` decide what fufu refuses an agent, so a write to either through the [`ff mcp`](mcp.md) tool is refused with `usage/mcp-policy-write`. That covers `--unset` too, which lowers the tier by taking the value away.
