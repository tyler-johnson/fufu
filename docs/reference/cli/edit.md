...
The branch you came from stays exactly where it stands, its commits waiting ahead. [`ff done`](done.md) amends the commit with what you changed and replays them onto it. A branch name is a switch instead — the one available reading is taken and announced. Your open change parks where you stood and comes back when the session ends. [`ff resolve`](resolve.md) opens its session the same way, over a held rewrite's conflicts.
