...
Remove integrations installed by [`ff hook`](hook.md). With no slugs, report detected integrations and ask; name clients or shells to remove those, or use `--all` for all detected integrations.
...
Fufu removes managed JSON commands, marked shell lines, the owned Claude Code and Codex plugin directories with the skill inside each, OpenCode's plugin file when fufu wrote it and its skill directory, and fufu's entry in the Codex marketplace file. JSON commands matching current or retired fufu spellings are managed even when pasted by hand. Unrelated settings, other marketplace entries, and hand-written shell equivalents remain. Cursor's old managed MCP registration is removed; unrelated commands remain.
