...
Agent hooks attempt capture before evaluating policy for received raw Git calls. Only Claude Code emits pre-tool coaching or denial replies; its client must enforce the denial. Codex, Qwen Code, OpenCode, and Cursor capture and tally recognized writes but emit no policy reply, including under strict.
