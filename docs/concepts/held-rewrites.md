...
When a step conflicts, nothing is touched. No ref moves, no half-applied tree reaches the working directory, no rebase sits stopped in the repository. Both inputs stay ordinary git commits.
...
Nothing moves when the session opens. Your branch stays put, a [parked change](changes.md) waits where it was, and the hold stays, because it is what the session is resolving.

Fix the markers, then `ff done` lands it. Each resolution is folded back into the step that wrote it, the chain re-runs in memory, and the whole rebased stack lands at once — refs move one time, every landed commit clean, no conflicted state ever existing in the graph.
