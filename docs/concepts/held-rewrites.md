...
jj gets away with parkable conflicts only because it pairs deferral with relentless disclosure, and holds inherit all three of its disciplines. First, a hold is announced at creation: the verb says what conflicts and where before it exits. Second, a hold is pinned until it is gone: `ff status` shows a `held:` line naming the verb, the commit it stopped at, the conflicting files, and the way out, on every render until the rewrite lands or is abandoned — and once a session is open, a `resolving:` line stands above it, because markers in your working tree are the more urgent fact. `ff branch list` marks the held branch the same way it marks an unfinished session, so standing work is visible wherever branches are listed. Third, exits are blocked, which is the next section.
